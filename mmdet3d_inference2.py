...
def ensure_bbox7(bbox):
    """
    Convert any bbox-like input to the canonical 7-element format:
    [x, y, z, dx, dy, dz, heading]

    If the bbox has extra fields (nuScenes), they are ignored.
    If the bbox has fewer than 7 values, raise an error.
    """
    arr = np.array(bbox, dtype=float).ravel()

    if arr.size < 7:
        raise ValueError(f"BBox must have ≥7 values, got {arr.size}")
